...
<html>
	...
	<body>
		...
		<s:if test="surveyList.isEmpty() == true">目前您没有任何调查项!</s:if >
		<s:else>
			<table>
				<tr>
				...
				<s:iterator value="surveyList" >
					...
					<tr>
						...
						<td><s:a action="survey-designSurvey?sid=%{#sId}" cssClass="aList">设计</s:a></td>
						...
						<td><s:a action="survey_toggleStatus?sid=%{#sId}" cssClass="aList">打开/关闭</s:a></td>
						<td><s:a action="survey_clearAnswer?sid=%{#sId}" cssClass="aList">清除调查</s:a></td>
						<td><s:a action="survey_delSurvey?sid=%{#sId}" cssClass="aList">删除</s:a></td>
					</tr>
				</s:iterator>
			</table>
		</s:else>
